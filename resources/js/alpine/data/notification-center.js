...
export function notificationCenter($wire) {
    return {
        ...
        init() {
            if (!window.Alpine) {
                console.error('[Notification Center] Alpine.js is not available');
                return;
            }

            const store = window.Alpine.store('notifications');
            if (!store) {
                console.error('[Notification Center] Notifications store is not available');
                return;
            }
            ...
            this.unsubscribe = store.subscribe((eventName) => {
                ...
                if ($wire && typeof $wire.$refresh === 'function') {
                    try {
                        ...
                        console.error('[Notification Center] Error refreshing component:', error);
                    }
                }
            });
        },
...
export function notificationDropdown($wire) {
    return {
        ...
        init() {
            if (!window.Alpine) {
                console.error('[Notification Dropdown] Alpine.js is not available');
                return;
            }

            const store = window.Alpine.store('notifications');
            if (!store) {
                console.error('[Notification Dropdown] Notifications store is not available');
                return;
            }
            ...
            this.unsubscribe = store.subscribe((eventName) => {
                ...
                if ($wire && typeof $wire.$refresh === 'function') {
                    try {
                        ...
                        console.error('[Notification Dropdown] Error refreshing component:', error);
                    }
                }
            });
        },
