...
new class extends Component {
    ...
    public function resetVerification(): void
    {
        ...
        $this->dispatch('$parent.resetVerification');
    }
}; ?>

<dialog id="two_factor_modal" class="modal">
    <div class="modal-box max-w-md">
        <div class="flex flex-col items-center space-y-4 mb-6">
            ...
            <h3 class="font-bold text-lg text-base-content">{{ $modalConfig['title'] ?? '' }}</h3>
            <p class="text-base-content/70 text-center">{{ $modalConfig['description'] ?? '' }}</p>
        </div>
        ...
        @if ($showVerificationStep)
            <div class="space-y-4">
                ...
                <div class="flex gap-2">
                    <button type="button" wire:click="resetVerification" class="btn btn-outline flex-1">
                        {{ __('Back') }}
                    </button>
                    <button type="button" wire:click="confirmTwoFactor" x-bind:disabled="$wire.code.length < 6"
                        class="btn btn-primary flex-1" :disabled="$wire.code.length < 6">
                        {{ __('Confirm') }}
                    </button>
                </div>
            </div>
        ...
        <div class="modal-action">
            <button type="button" class="btn btn-primary" wire:click="$parent.showVerificationIfNecessary"
                :disabled="$wire.$parent.errors->has('setupData')">
...
<form method="dialog" class="modal-backdrop">
    <button type="button" onclick="$wire.$parent.closeModal()">close</button>
</form>
</dialog>
