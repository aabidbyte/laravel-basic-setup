...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <head>
        ...
        <title>{{ __('emails.email_change_verification.subject', ['app' => $appName]) }}</title>
    </head>
    ...
    <body
        ...
        <div
            ...
            <h2 style="color: #333; margin-top: 0;">
                {{ __('emails.email_change_verification.greeting', ['name' => $user->name]) }}</h2>

            <p>{{ __('emails.email_change_verification.intro') }}</p>

            <p>{{ __('emails.email_change_verification.instructions') }}</p>
            ...
            <p style="color: #666; font-size: 14px;">
                {{ __('emails.email_change_verification.expires', ['days' => $expiresInDays]) }}
            </p>

            <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 20px 0;">

            <p style="color: #999; font-size: 12px;">
                {{ __('emails.email_change_verification.link_fallback') }}
            </p>
            ...
            <p style="color: #999; font-size: 12px; text-align: center;">
                {{ __('emails.email_change_verification.footer') }}
            </p>
        </div>
    </body>
