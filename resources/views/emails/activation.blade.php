<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">

    <head>
        <meta charset="utf-8">
        <meta name="viewport"
              content="width=device-width, initial-scale=1">
        <title>{{ __('emails.activation.subject', ['app' => $appName]) }}</title>
    </head>

    <body
          style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div
             style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">{{ $appName }}</h1>
        </div>

        <div
             style="background: #ffffff; padding: 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
            <h2 style="color: #333; margin-top: 0;">{{ __('emails.activation.greeting', ['name' => $user->name]) }}</h2>

            <p>{{ __('emails.activation.intro') }}</p>

            <p>{{ __('emails.activation.instructions') }}</p>

            <div style="text-align: center; margin: 30px 0;">
                <a href="{{ $activationUrl }}"
                   style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-decoration: none; padding: 15px 30px; border-radius: 5px; font-weight: bold; font-size: 16px;">
                    {{ __('emails.activation.button') }}
                </a>
            </div>

            <p style="color: #666; font-size: 14px;">
                {{ __('emails.activation.expires', ['days' => $expiresInDays]) }}
            </p>

            <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 20px 0;">

            <p style="color: #999; font-size: 12px;">
                {{ __('emails.activation.link_fallback') }}
            </p>
            <p style="color: #667eea; font-size: 12px; word-break: break-all;">
                {{ $activationUrl }}
            </p>

            <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 20px 0;">

            <p style="color: #999; font-size: 12px; text-align: center;">
                {{ __('emails.activation.footer') }}
            </p>
        </div>
    </body>

</html>
