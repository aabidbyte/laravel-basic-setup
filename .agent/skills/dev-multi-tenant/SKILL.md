...
Multi-tenant isolation is 100% based on `tenantId` from JWT → Laravel tenant middleware resolution → database-per-tenant. Tenant resolution via `stancl/tenancy` or custom `TenantService` resolves tenant-specific database connections. `organization_id` is NOT part of multi-tenant.
