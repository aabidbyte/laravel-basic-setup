...
Source: https://tenancyforlaravel.com/docs/v3/cached-lookup
...
If you're using multiple databases, you may want to avoid making a query to the central database on <strong>each tenant request</strong> — for tenant identification. Even though the queries are very simple, the app has to establish a connection with the central database which is expensive.
...
<ul>
<li>`DomainTenantResolver` (also used for subdomain identification)</li>
<li>`PathTenantResolver`</li>
<li>`RequestDataTenantResolver`</li>
</ul>
...
```
<code class="language-php">// TenancyServiceProvider::boot()
...
```
<code class="language-php">app(\Stancl\Tenancy\Resolvers\DomainTenantResolver::class)-&gt;invalidateCache($tenant);
</code>
```
