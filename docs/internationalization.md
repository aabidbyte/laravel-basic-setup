...
The command uses a **strict service-based architecture** to ensure reliability:

1.  **`TranslationScanner`**: Scans the codebase using Regex patterns to find all `__('key')`, `@lang('key')`, and `trans('key')` usage.
2.  **`LocaleManager`**:
    -   Uses default locale (`en_US`) as the source of truth.
    -   Syncs missing keys to other locales.
    -   **Strictly preserves** the short battery syntax `[...]` in generated files.
3.  **`DynamicKeyResolver`**:
    -   Identifies dynamic keys like `permissions.{$action}`.
    -   Attempts to auto-resolve them using `config/translation-resolvers.php`.
    -   Expands them into concrete keys (e.g., `permissions.create`, `permissions.edit`).
4.  **`TranslationPruner`**: Optionally removes unused keys (respecting protected files).

### Example Output

```
Scanning codebase for translation usage...
Found 575 translation keys in codebase.
Syncing default locale with missing keys...
Syncing locales...
...
| Metric                 | Count |
+------------------------+-------+
| Keys found in codebase | 575   |
| Keys added to locales  | 3     |
| Keys pruned            | 0     |
| Files updated          | 1     |
+------------------------+-------+
```
