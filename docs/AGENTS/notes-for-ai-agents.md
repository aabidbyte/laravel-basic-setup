...
29. **Use project scripts first** - Always prefer Composer scripts and package scripts when they exist (`composer run pint`, `composer run migrate-fresh`, `composer test:*`, `npm run build`, `npm run format:*`) instead of direct lower-level commands. Use direct commands only for narrow diagnostics or tasks without a defined script.
