...
test('resolves tags in layout and template', function () {
    ...
    expect($rendered->html)->toContain('<title>SuperApp</title>');
    expect($rendered->text)->toContain('App: SuperApp');

    // 5. Verify Template Tags Resolved
    expect($rendered->html)->toContain('Welcome User123');
    expect($rendered->text)->toContain('Welcome User123');

    // 6. Verify Subject Tag
    expect($rendered->subject)->toBe('Hello SuperApp');
});
